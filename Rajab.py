...
jika platform.python_version (). split ( ' . ' ) [ 0 ] ! =  ' 2 ' :
	cetak ( ' ! m [!] Kamu menggunakan versi python % s menggunakan versi 2.xx ' % v (). split ( '  ' ) [ 0 ])
	sys.exit ()
...
fid_bteman = []
fid_bgroup = []
kelas  mt ( threading . Thread ):
	def  __init__ ( self , i , p ):
		threading. Bayangkan. __init__ ( mandiri )
		self .id = i
		diri .a =  0
		self .p = p
	 pembaruan def ( mandiri ):
		kembali  diri .a, diri .id
	def  run ( mandiri ):
		coba :
